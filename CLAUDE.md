...
4. **Fixed supply is real.** No minting exists (mint renounced at fair launch). The ONLY codepath that credits $AMP to a player is the **champions'-purse** prize payout, funded by the treasury and the monthly buyback's purse half (creator-fee funded, on by default via `CREATOR_REWARDS_ENABLED`) — never by minting, and within its published cap. Never fabricate tokens.
