...
**The constraint (locked):** $AMP is an ERC-20 on Robinhood Chain — fixed 1B supply, mint renounced, LP permanently locked. No reward can ever be printed. Value accrues only through buy/hold pressure, burns, locks, and (flag-gated) buybacks against fixed supply.
...
```
   ...
   (flag-gated, CREATOR_REWARDS_ENABLED — off until confirmed:
    hood.fun ETH creator rewards → monthly buyback, half burn /
    half champions' purse. Off ⇒ no buyback, no change to the above.)
```

The **demand loop** is powered by the game being worth belonging to — and by the standing requirement to **hold 1,000 $AMP** to stay Charged. The **supply loop** only ever burns, locks, or holds: every spend is $AMP-only, 30% burned on-chain at the till and 70% to a treasury wallet that never market-sells (Comms rule 5); the treasury funds the champions' purse and discretionary burns. **Supply only ever falls.** The design stays coherent even at zero trading volume — nothing depends on the buyback, which is off (`CREATOR_REWARDS_ENABLED=false`) until hood.fun confirms creator-fee volume; if flipped on, it adds a monthly buyback sourced from ETH creator rewards (half burn / half purse), never a price promise.
...
- **Buyback — flag-gated (`CREATOR_REWARDS_ENABLED`, off for now):** hood.fun has not confirmed creator-fee volume, so there is **no buyback** at launch. If confirmed later, flipping the flag re-adds a **monthly buyback sourced from ETH creator rewards**, executed as randomized TWAP across the month (a published lump-sum buy on a thin pool is an MEV/frontrunning target), split **50% burn / 50% champions' purse** until the purse cap, then 100% burn. While the flag is off, none of this runs and nothing else changes.
...
- **Hard caps (both required):** purse balance ≤ 5M $AMP (0.5% of supply) at any time, **and total payouts ≤ 10M $AMP (1% of supply) per rolling year** — the annual cap is what actually bounds flow (a balance cap alone allows unlimited drain-and-refill). Funded by the treasury (the 70% share) and, only if `CREATOR_REWARDS_ENABLED` is ever set, the buyback's purse half (C4); when the purse is full, any buyback burns 100%.
...
5. Champions' purse: balance ≤ 5M; rolling-year payouts ≤ 10M; funded by the treasury (and the buyback's purse half only if `CREATOR_REWARDS_ENABLED` is set).
...
The token launches on hood.fun *now*; full utility ships at M4, months away. Unmanaged, that gap reads as abandonment and the token arrives as an illiquid ghost. Policy for the build period: (1) publish the treasury + token addresses from day one, and — only if `CREATOR_REWARDS_ENABLED` is set — claim hood.fun ETH creator rewards into the disclosed accounting; (2) publish the City Ledger from month one, even when it only reports "nothing sold, N burned from early registrations"; (3) ship **one small honest utility early** at M2 — the vanity registry (reserved Spark names + founder nameplates, standard 30/70 burn/treasury split) — so the token has a live, logged sink before the premium layer lands; (4) communicate the build openly under the comms rules — progress, never price.
...
| **M4 — Economy + $AMP** | The **1,000-$AMP token gate** (SIWE + `balanceOf`) + Charged membership, season pass, stall deed auctions, Cosmetic Foundry, Charge Locks (non-custodial), $AMP-only payments (30/70) + oracle pricing, treasury + City Ledger + flag-gated buyback, full C6 invariant suite | 4–7 weeks |
...
> **v3 supersession note.** The economic model below was written for v2's **Dynamo Bond bridge** and **two-rail** design on the previous chain. v3 replaces both: $AMP now lives on **Robinhood Chain** (ERC-20, hood.fun), access is the **1,000-$AMP token gate** (hold the key, non-custodial), and the economy is **$AMP-only** (30% burn / 70% treasury, treasury never sells; buyback is flag-gated off). The *lessons* below (prove retention first, no wage, de-gambled Coil, small-population market, transparency) are unchanged and still load-bearing — only the Bond mechanism and the second rail were removed. Entries that described the Bond or the second rail have been updated in place.
...
**3. Demoted buyback-and-burn from engine to hygiene (v3: to flag-gated, off).** v1's flywheel leaned on creator-fee-funded buybacks as a core value driver. The 2026 evidence is brutal: a major launchpad spent ~$370M buying back its own token and it stayed ~80% below peak, capitulating in April 2026 to a programmatic model; Gunzilla's 30%-of-revenue GUN buyback didn't prevent an ~87% drawdown; and launchpad token trading volume (hence creator fees) decays toward zero for almost all tokens within weeks. Buybacks retime demand, they don't create it. So the buyback is never a pillar: v2 sized it as automated supply hygiene, and **v3 gates it off entirely** behind `CREATOR_REWARDS_ENABLED` until hood.fun confirms creator-fee volume — if ever flipped on, it's a randomized-TWAP monthly buyback from ETH creator rewards (half burn / half champions' purse), never a price promise. The studio never needs a token sale because the **treasury (the 70% of every $AMP spend) never sells** — the design stays solvent at zero token volume without any fiat rail.
...
- launchpad creator-fee decay (2026): coinmarketcap.com/academy (Dynamic Fees V1) · crypto.news (Jan 2026 fee overhaul) · dextools.io/news (graduation-collapse fee analysis, 2026)
