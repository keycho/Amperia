# ART DIRECTION — "Cozy Salvage-Punk" *(v2.3 — "Night Market at Night" color revision)*

> **v2.3 revision (supersedes conflicting lines below):** the world's base mood shifts from *golden dusk haze* to **true night, warm-lit** — a dark ink-plum world where warm light sources (amber lanterns, rose signs, golden windows, the Dynamo) carry the scene as pools and points against the dark, with teal/cyan as secondary accents. Coziness now lives in the warm light itself, campfire-logic: warm pools against real darkness. The palette hexes are unchanged; their *usage weights* changed — `#35284F`/`#1E1930` own the base world, `#FFD9A0` becomes pools (never a global wash), the neons run hot with bloom. Sections §1–§3 and §7 below are amended by this note where they conflict.
...
**Palette rules *(usage weights revised in v2.3)*:**
- **Big areas use the dark values** (`#35284F`, `#4E4560`, shadowed toward `#1E1930`). The mid-values (`#6B5E70`, `#9A8574`) now belong to *objects and lit surfaces* — decking under a lantern, a stall counter, a rug in a light pool — so everything touched by light sits visibly lighter than the dark base world. Pure saturated neon remains an *accent*, never a fill.
...
## 3. Lighting *(rewritten in v2.3 — "night market at night")*

- **The world is dark; the light is warm.** Base scene sits in deep ink-plum (`#35284F` toward `#1E1930` in shadow — never large pure-black fills). There is NO global warm wash. Warmth arrives only as light *sources*: lantern pools, golden windows, sign glow, the Dynamo's halo.
- **Warm light dominates, cool light accents — roughly 70/30.** Amber `#FFB84D` and rose `#FF6F91` are the city's light; teal `#2FD3B8` / cyan `#5BC0FF` punctuate (interactables, screens, coolant). If a screenshot's light reads cold overall, it's wrong.
- **Light pools are the composition.** Every lamp, stall, and window casts a visible warm pool (`#FFD9A0` gradient) on the ground; the walkable world is a rhythm of warm pools connected by genuinely dark stretches. Players navigate light to light — that IS the cozy feeling, campfire-logic.
- **Hot bloom on all emissives.** Signs, string bulbs, node glints, and the Dynamo glow visibly against the dark floor. Emissives are the contrast engine of the entire look.
- **Wet-sheen ground, lightly:** tiles carry a subtle reflective glaze that catches nearby light pools and neon — damp and glinting, not drenched and gloomy.
- **Gentle haze, never murk:** distant lights bloom softly; the parallax skyline reads as dim lit windows against the dark sky.
- **No day cycle.** It is always night in Amperia; mood variety comes from seasonal re-lights and the Citywide Charge (§11), which modulate *how much* of the city is lit and in which accent hue — never from daylight.
...
1. **Dark ground, lighter subjects** *(v2.3)*: the floor is the darkest large surface in play; characters, items, nodes, and lit props always sit clearly lighter/warmer, so they pop by value. Interactables additionally get their neon cue. Never let the ground brighten to where subjects stop separating.
...
The style target is a uniform chunky-voxel world where cohesion comes from every asset obeying the same mechanical build rules — not from asset quality. Mixing construction styles (textured packs + smooth procedural + stock-colored sprites) is what makes a scene read as placeholder soup; these rules exist so that can never happen again.

**Technique-reference disclaimer:** the *construction method* references voxel MMOs like Kintara; the *visual identity* must never resemble them. Kintara is daylight, grass, and pastoral fantasy. Amperia is perpetual dusk, plum-dark plating, neon signage, steam, and machine-city clutter — Part I's palette and "what it is NOT" rules already guarantee the divergence. If a screenshot could be mistaken for a sunny voxel village, the *lighting and content* are wrong, no matter how correct the voxel grammar is.
...
5. **Contrast = a dark world with warm-lit subjects** *(v2.3)*: ground sits deep ink-plum, objects sit clearly lighter (mid-values, warm where lit), light pools and emissives run hot with bloom. Warm light ≈ 70% / cool accents ≈ 30%. Big flat color planes that read at a glance.
